...
# largely the same as V3, yet this strict version applies more relaxed rules for labelling
# a base as unassigned; as long as the estimate is less than 1, the base is labelled as unassigned
# This parallel version is designed for wrapper
...
parser.add_option("--chrom",action="store",type="string",dest="chromosome",help="Name of the chromosome currently being processed")
...
outFile = open("{}.w{}.singlebase.temp".format(options.chromosome, width),'w')
outFile2 = open("{}.w{}.block.temp".format(options.chromosome, width),'w')
...
def process_chrom(array1, array2, outfile_single, outfile_block, chrom):
    ...
    for i in list(range(array1.size)):
        ...
        if count1_adjusted -1 <= tol and count2_adjusted -1 <= tol:
            count_un += 1
            outfile_single.write("{}\t{}\t{}\n".format(chrom, i+1, 'unassigned'))
            assign = 'unassigned'
        elif count1_adjusted -1 <= tol and count2_adjusted-1 > tol:
            count_G2 += 1
...
logFile = open("{}.log.temp".format(options.chromosome),'w')
logFile.write("Assignment of {} Done\n".format(options.chromosome))
total = count_G1 + count_G2 + count_ambi + count_un
logFile.write("A total of {} bases processed\n".format(total))
logFile.write("{}({:.4f}%) bases are assigned to {}\n".format(count_G1,100*count_G1/total,subG1_name))
logFile.write("{}({:.4f}%) bases are assigned to {}\n".format(count_G2,100*count_G2/total,subG2_name))
logFile.write("{}({:.4f}%) bases are ambiguous\n".format(count_ambi,100*count_ambi/total))
logFile.write("{}({:.4f}%) bases are unassigned\n".format(count_un,100*count_un/total))
